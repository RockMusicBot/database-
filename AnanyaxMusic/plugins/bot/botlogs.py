...
from AnanyaxMusic.utils.database import add_served_chat, get_assistant, delete_served_chat

@akash.on_message(filters.new_chat_members, group=-10)
async def join_watcher(_, message):
    try:
        userbot = await get_assistant(message.chat.id)
        for member in message.new_chat_members:
            if member.id == akash.id:
                count = await akash.get_chat_members_count(message.chat.id)
                username = message.chat.username if message.chat.username else "Private Group"

                invite_link = ""
                try:
                    if not message.chat.username:
                        link = await akash.export_chat_invite_link(message.chat.id)
...
@akash.on_message(filters.left_chat_member, group=-12)
async def on_left_chat_member(_, message):
    try:
        userbot = await get_assistant(message.chat.id)
        left_member = message.left_chat_member

        if left_member and left_member.id == (await akash.get_me()).id:
            ...
            await akash.send_message(LOGGER_ID, text=caption)
